...
- [Random Hierarchy Model (RHM)](papers/Pretraining/random-hierarchy-model.md) — 「なぜ深層ネットは高次元データを少数例で学べるか」を合成タスク **RHM**（言語・画像の階層構造を模した PCFG）で解析。深い CNN のサンプル複雑度は **P\* = n_c · m^L**（n_c=クラス数, m=multiplicity, L=深さ）で、入力次元 d=s^L に対し**多項式**＝次元の呪いを回避（浅いネットは指数的＝深さが本質）。学習は「**同義（synonyms）の交換に不変な表現**を段階構築」する過程で、低レベル特徴とクラスの相関がノイズを超える点が閾値。本リポジトリの RHM 系（latent-sample-complexity / language-structure）の起点 (Petrini, Cagnetta, Tomasini, Favero, Wyart, 2023 / EPFL) `✅ Physical Review X 2024`
- [言語構造の獲得理論](papers/Pretraining/language-structure-acquisition.md) — RHM を LM に展開。**PCFG 上で token-token 相関を解析的に導出**し、相関 range が長いほど深い隠れ変数に対応。**有限の訓練集合は相関解像度を effective range に制限**し、range はデータ量とともに伸びる → **データを増やすほど文法のより深い表現を獲得**。テスト損失のスケーリング則が**文脈窓長に依存**することを予測し Shakespeare/Wikipedia で検証。経験的スケーリング則に機序的説明を与える (Cagnetta & Wyart, 2024 / EPFL) `✅ NeurIPS 2024`
- [BERT Rediscovers the Classical NLP Pipeline](papers/Pretraining/bert-rediscovers-nlp-pipeline.md) — **edge probing** で BERT 各層を解析し、古典 NLP パイプライン（**POS → 構文 → NER → 意味役割 → 照応**）が期待順に層に局在＝**下位層に統語・上位層に意味**を実証。center-of-gravity と cumulative scoring の2指標、上位→下位の修正を行う「動的パイプライン」も観測。[言語構造の獲得理論](papers/Pretraining/language-structure-acquisition.md) の経験的対応物 (Tenney, Das, Pavlick, 2019 / Google) `✅ ACL 2019`
- [Does BERT Rediscover a Classical NLP Pipeline?](papers/Pretraining/does-bert-rediscover-nlp-pipeline.md) — 上記の批判的再検証。**層分離（表層/統語/意味）には決定的な経験的支持が乏しく** probe 方法論・指標に敏感と指摘。**GridLoc**（トークン位置・訓練ラウンド・乱数シードを考慮）を提案し、層深さに頼らないより強い規則性を検出。「BERT の言語構造は実在するが層深さは最良の説明軸でない」 (Niu, Lu, Penn, 2022 / Toronto大) `✅ COLING 2022`
