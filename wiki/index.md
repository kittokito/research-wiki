...
- [Transformers are Inherently Succinct](papers/Architecture/transformers-are-inherently-succinct.md) — Transformer の表現力を **succinctness（簡潔性）** で測り、`Transformer > LTL > RNN > 有限オートマトン` の簡潔性階層を構成的に証明。固定精度 **UHAT（Masked Unique Hard-Attention Transformer）** は **LTL より指数関数的に**（Theorem 14）、**有限オートマトンより二重指数関数的に**（Theorem 16）、**固定精度 RNN より指数関数的に**（Corollary 17）簡潔。逆に LTL→UHAT は多項式変換でブローアップなし（Prop 15）。さらに UHAT/B-RASP の **非空性（Theorem 5）・等価性（Theorem 18）判定は EXPSPACE-complete** で検証が本質的に困難（先行研究の UHAT→LTL 二重指数変換を指数時間に改善した Prop 12 が上界の鍵）。並列 attention 構造が逐次モデル・論理式に対する簡潔性優位の源泉。前提は unique hard-attention で softmax/浮動小数点は対象外 (Bergsträßer, Cotterell, Lin, 2025 / RPTU Kaiserslautern × ETH Zürich) `✅ ICLR 2026 (Oral, Outstanding Paper)`
...
- [RAGとAgentic Searchの戦争を終わらせに来た!!!](papers/Agent_ToolUse/rag-vs-agentic-search.md) — 「RAG は終わった」言説を**用語の定義ズレに起因する疑似論争**として整理。RAG は「ベクトル検索前提」から「外部データ参照で生成強化」へ広義化（MS/AWS/NVIDIA 共通）、**Agentic Search の本質はファイルシステム探索ではなく「推論で複数回検索を反復する戦略」**で手段（vector / grep）は不問。**ベクトル検索 RAG は死んでいない**（「何でもベクトル検索」から脱しただけ）。Agentic 検索の3留意点（遅延 / コンテキスト消費 / 検索履歴のノイズ化）、**CAG**（大コンテキスト＋キャッシュ、フラッグシップ＋少数ファイルで有効・小型/多タスク/大ドキュメントで破綻）、ファイルシステム探索 RAG（事前準備不要・高速、探索空間が狭くないと破綻）、Karpathy の **LLM Wiki**（Graph RAG をファイルシステムで完結）を比較。結論「対象データの性質・規模・タスクで使い分ける」 (Hirosato Gamo, 2026 / Microsoft・Zenn) `— blog`
