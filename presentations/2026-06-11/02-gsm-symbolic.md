...
*同一構造の問題で、名前だけ変える(Names)・数値だけ変える(Numbers)・両方変える(Both)と精度分布が動く。Gemma2-9b-it の例で GSM8K 87.0 → Names 88.6 → Numbers 83.1 → Both 79.1。点線が元の GSM8K スコア。**名前だけの変更はほぼ影響しない（88.6）が、数値を変えると低下（83.1）、両方変えると最も低下（79.1）し、分布も広がる**。*
